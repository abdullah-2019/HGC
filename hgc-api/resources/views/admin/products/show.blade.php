...
<div class="space-y-6" x-data="{ activeTab: 'overview' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white">{{ $product->name_en }}</h1>
                <p class="text-sm text-gray-400 mt-1">{{ $product->slug }}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.products.edit', $product) }}"
               class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 focus:ring-4 focus:ring-primary-900 transition-colors">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 flex flex-wrap items-center gap-3">
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border {{ $availabilityColors[$product->availability] ?? 'bg-gray-700 text-gray-300' }}">
            {{ $availabilityLabels[$product->availability] ?? $product->availability }}
        </span>
        @if($product->is_active)
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-900/50 text-green-400 border border-green-800">Active</span>
        @else
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-900/50 text-red-400 border border-red-800">Inactive</span>
        @endif
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @if($product->hero_image_url)
                <div class="relative group">
                    ...
                    <span class="absolute top-2 left-2 px-2 py-0.5 text-xs font-medium bg-primary-600 text-white rounded">Hero</span>
                </div>
            @endif
            @if($product->thumbnail_url)
                <div class="relative group">
                    ...
                    <span class="absolute top-2 left-2 px-2 py-0.5 text-xs font-medium bg-gray-600 text-white rounded">Thumb</span>
                </div>
            @endif
        ...
        <div class="border-b border-gray-700">
            <nav class="flex space-x-1 p-1 overflow-x-auto" aria-label="Tabs">
                <button @click="activeTab = 'overview'"
                        :class="activeTab === 'overview' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'"
                        class="px-4 py-2.5 text-sm font-medium rounded-lg transition-colors whitespace-nowrap">
                    Overview
                </button>
                <button @click="activeTab = 'details'"
                        :class="activeTab === 'details' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'"
                        class="px-4 py-2.5 text-sm font-medium rounded-lg transition-colors whitespace-nowrap">
                    Details
                </button>
                <button @click="activeTab = 'specifications'"
                        :class="activeTab === 'specifications' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'"
                        class="px-4 py-2.5 text-sm font-medium rounded-lg transition-colors whitespace-nowrap">
                    Specifications
                </button>
        ...
        <div class="p-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="bg-gray-700/50 rounded-lg p-3">
                                <p class="text-xs text-gray-500 mb-1">English</p>
                                <div class="text-white text-sm prose prose-invert max-w-none">{!! $product->overview_en !!}</div>
                            </div>
                            @endif
            ...
            <div x-show="activeTab === 'details'" x-cloak>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="bg-gray-700/50 rounded-lg p-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Price Range</p>
                        <p class="text-white font-medium">{{ $product->price_range ?? 'N/A' }} {{ $product->currency }}</p>
                    </div>
                    <div class="bg-gray-700/50 rounded-lg p-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Unit</p>
                        <p class="text-white font-medium">{{ $product->unit ?? 'N/A' }}</p>
                    </div>
                    <div class="bg-gray-700/50 rounded-lg p-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Availability</p>
                        <p class="text-white font-medium">{{ $availabilityLabels[$product->availability] ?? $product->availability }}</p>
                    </div>
                    <div class="bg-gray-700/50 rounded-lg p-4">
                ...
                <div class="mt-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($product->applications as $app)
                            <span class="px-3 py-1.5 text-sm bg-gray-700 text-gray-300 rounded-lg border border-gray-600">{{ $app }}</span>
                        @endforeach
                    </div>
                </div>
                @endif
                ...
                <div class="mt-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($product->packaging as $pkg)
                            <span class="px-3 py-1.5 text-sm bg-gray-700 text-gray-300 rounded-lg border border-gray-600">{{ $pkg }}</span>
                        @endforeach
                    </div>
                </div>
                @endif
