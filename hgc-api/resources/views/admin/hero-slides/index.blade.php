        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-800">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Image
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Badge
                            (EN)</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Ken Burns
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Order
                        </th>
                    ...
                    @forelse($slides as $slide)
                        <tr class="hover:bg-gray-700/30 transition-colors">
                            <td class="px-4 py-3 whitespace-nowrap">
                                <img src="{{ asset('storage/' . ltrim($slide->image, '/')) }}" 
                                    class="h-10 w-16 object-cover rounded border border-gray-600">
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-700 text-gray-300 border border-gray-600">
                                    {{ $slide->ken_burns }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                @if ($slide->is_active)
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-900/50 text-green-400 border border-green-800">
                                        Active
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-700 text-gray-400 border border-gray-600">
                                        Inactive
                                    </span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-gray-500 text-sm">
                                No hero slides found. <a href="{{ route('admin.hero-slides.create') }}"
                                    class="text-primary-400 hover:underline">Create one</a>.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
