...
@section('content')
    <div class="max-w-5xl mx-auto">
        ...
        <form action="{{ route('admin.hero-slides.update', $heroSlide) }}" method="POST" class="bg-gray-800 border border-gray-700 rounded-lg shadow-sm">
            @csrf
            @method('PUT')
            ...
            <div class="p-4 sm:p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="image"
                               value="{{ old('image', $heroSlide->image) }}"
                    ...
                    <div>
                        ...
                        <select name="ken_burns"
                                class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">
                            <option value="zoom-in" {{ old('ken_burns', $heroSlide->ken_burns) == 'zoom-in' ? 'selected' : '' }}>Zoom In</option>
                            <option value="zoom-out" {{ old('ken_burns', $heroSlide->ken_burns) == 'zoom-out' ? 'selected' : '' }}>Zoom Out</option>
                            <option value="pan-right" {{ old('ken_burns', $heroSlide->ken_burns) == 'pan-right' ? 'selected' : '' }}>Pan Right</option>
                            <option value="pan-left" {{ old('ken_burns', $heroSlide->ken_burns) == 'pan-left' ? 'selected' : '' }}>Pan Left</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="badge_dari"
                               value="{{ old('badge_dari', $heroSlide->badge_dari) }}" dir="rtl"
                               class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <textarea name="title_en" rows="3"
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">{{ old('title_en', is_array($heroSlide->title_en) ? implode("\n", $heroSlide->title_en) : '') }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="title_dari" rows="3" dir="rtl"
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">{{ old('title_dari', is_array($heroSlide->title_dari) ? implode("\n", $heroSlide->title_dari) : '') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-2">Title (Pashto)</label>
                        <textarea name="title_pashto" rows="3" dir="rtl"
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">{{ old('title_pashto', is_array($heroSlide->title_pashto) ? implode("\n", $heroSlide->title_pashto) : '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <textarea name="subtitle_en" rows="2"
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">{{ old('subtitle_en', $heroSlide->subtitle_en) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="subtitle_dari" rows="2" dir="rtl"
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">{{ old('subtitle_dari', $heroSlide->subtitle_dari) }}</textarea>
                    </div>
                    <div>
                        ...
                        <textarea name="subtitle_pashto" rows="2" dir="rtl"
                                  class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-colors">{{ old('subtitle_pashto', $heroSlide->subtitle_pashto) }}</textarea>
                    </div>
                </div>
            ...
            <div class="px-4 sm:px-6 py-4 bg-gray-800/50 border-t border-gray-700 flex items-center justify-end gap-3">
                ...
                <button type="submit"
                        class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors">Update Slide</button>
            </div>
        </form>
    </div>
@endsection