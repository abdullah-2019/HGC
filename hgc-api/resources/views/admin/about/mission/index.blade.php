        ...
        @if ($mission)
            <div class="max-w-7xl mx-auto">
                ...
                <div class="flex flex-row items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 class="text-3xl font-bold text-white">About Mission</h1>
                        <p class="text-gray-400 mt-2">Complete mission content preview</p>
                    </div>
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div class="p-6 sm:p-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                            <div class="bg-gray-800/60 border border-gray-700/50 rounded-xl p-5 hover:border-gray-600 transition-colors">
                        ...
                        <div class="mb-8">
                            ...
                            {{-- English Content --}}
                            <div id="content-en" class="bg-gray-800/40 border border-gray-700/40 rounded-2xl p-6 sm:p-8 space-y-6">
                                <div>
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">Title</label>
                                    <p class="text-white text-2xl sm:text-3xl font-bold leading-tight">{{ $mission->title_en }}</p>
                                </div>
                                <div>
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">Description</label>
                                ...
                                <div class="bg-gray-900/60 border-l-4 border-blue-500 rounded-r-xl p-5">
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">Quote</label>
                                    <p class="italic text-gray-300 text-lg leading-relaxed">"{{ $mission->quote_text_en }}"</p>
                                </div>
                            </div>

                            {{-- Dari Content --}}
                            <div id="content-dari" class="hidden bg-gray-800/40 border border-gray-700/40 rounded-2xl p-6 sm:p-8 space-y-6" dir="rtl">
                                <div>
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">عنوان</label>
                                    <p class="text-white text-2xl sm:text-3xl font-bold leading-tight">{{ $mission->title_dari }}</p>
                                </div>
                                <div>
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">توضیحات</label>
                                ...
                                <div class="bg-gray-900/60 border-r-4 border-blue-500 rounded-l-xl p-5">
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">نقل قول</label>
                                    <p class="italic text-gray-300 text-lg leading-relaxed">"{{ $mission->quote_text_dari }}"</p>
                                </div>
                            </div>

                            {{-- Pashto Content --}}
                            <div id="content-pashto" class="hidden bg-gray-800/40 border border-gray-700/40 rounded-2xl p-6 sm:p-8 space-y-6" dir="rtl">
                                <div>
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">سرلیک</label>
                                    <p class="text-white text-2xl sm:text-3xl font-bold leading-tight">{{ $mission->title_pashto }}</p>
                                </div>
                                <div>
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">تشریح</label>
                                    <p class="text-gray-300 leading-relaxed text-base">{{ $mission->description_pashto }}</p>
                                </div>
                                <div class="bg-gray-900/60 border-r-4 border-blue-500 rounded-l-xl p-5">
                                    <label class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">اقتباس</label>
                                    <p class="italic text-gray-300 text-lg leading-relaxed">"{{ $mission->quote_text_pashto }}"</p>
                                </div>
                            </div>
                        </div>

                        {{-- MISSION POINTS --}}
                        @if ($mission->allPoints && $mission->allPoints->count() > 0)
                            <div class="mt-12">
                                <div class="flex items-center gap-3 mb-6">
                                    <span class="w-1.5 h-8 bg-gradient-to-b from-green-500 to-green-700 rounded-full"></span>
                                    <h2 class="text-2xl font-bold text-white">Mission Points</h2>
                                    {{-- FIX: Use allPoints->count() instead of points->count() --}}
                                    <span class="ml-auto px-3 py-1 rounded-full text-xs font-medium bg-gray-800 text-gray-400 border border-gray-700">{{ $mission->allPoints->count() }} items</span>
                                </div>

                                <div class="grid md:grid-cols-2 gap-5">
                                    @foreach ($mission->allPoints as $index => $point)
                                        <div class="group bg-gray-800/40 border border-gray-700/40 hover:border-gray-600 rounded-2xl p-6 transition-all hover:bg-gray-800/60">
                                            <div class="flex justify-between items-start mb-5">
                                                ...
                                                @if ($point->is_active)
                                                    <span class="text-xs px-3 py-3 rounded-full bg-green-500/10 text-green-400 border border-green-500/20 font-medium">Active</span>
                                                @else
                                                    <span class="text-xs px-3 py-3 rounded-full bg-gray-500/10 text-gray-400 border border-gray-500/20 font-medium">Inactive</span>
                                                @endif
                                            </div>
                                            <div class="space-y-4">
                                                <div>
                                                    <p class="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1">English</p>
                                                    <p class="text-gray-200 text-sm leading-relaxed">{{ $point->text_en }}</p>
                                                </div>
                                                <div class="border-t border-gray-700/50 pt-4" dir="rtl">
                                                    <p class="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1">دری</p>
                                                    <p class="text-gray-200 text-sm leading-relaxed">{{ $point->text_dari }}</p>
                                                </div>
                                                <div class="border-t border-gray-700/50 pt-4" dir="rtl">
                                                    <p class="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1">پښتو</p>
                                                    <p class="text-gray-200 text-sm leading-relaxed">{{ $point->text_pashto }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
