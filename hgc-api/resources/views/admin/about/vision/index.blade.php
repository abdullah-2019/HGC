    ...
    <div class="min-h-screen bg-gray-950 p-6">
        @if ($vision)
            <div class="max-w-7xl mx-auto">
                ...
                <div class="flex flex-row items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 class="text-3xl font-bold text-white">About Vision</h1>
                        <p class="text-gray-400 mt-2">Complete vision content preview</p>
                    </div>
                    <a href="{{ route('admin.about.vision.edit', $vision) }}"
                    ...
                    @if ($vision->image_url)
                        <div class="relative">
                            <img src="{{ asset('storage/' . ltrim($vision->image_url, '/')) }}"
                                class="w-full h-[320px] sm:h-[420px] object-cover" alt="Vision">
                            <div class="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/40 to-transparent">
                            </div>
                            <div class="absolute bottom-6 left-6 flex items-center gap-3">
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-semibold {{ $vision->is_active ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30' }} backdrop-blur-sm">
                    ...
                    <div class="p-6 sm:p-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                            <div
                                ...
                                <p class="text-white font-semibold text-lg">{{ $vision->section_label_en }}</p>
                            </div>
                            <div
                                class="bg-gray-800/60 border border-gray-700/50 rounded-xl p-5 hover:border-gray-600 transition-colors">
                                <div class="flex items-center gap-2 mb-2">
                                ...
                                <p class="text-white font-semibold text-lg">{{ $vision->sort_order }}</p>
                            </div>
                            <div
                                class="bg-gray-800/60 border border-gray-700/50 rounded-xl p-5 hover:border-gray-600 transition-colors">
                                <div class="flex items-center gap-2 mb-2">
                                ...
                                <p class="text-white font-semibold text-lg">{{ $vision->created_at->format('M d, Y') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mb-10">
                            <div class="flex items-center gap-2 mb-5">
                                <span class="w-1.5 h-8 bg-gradient-to-b from-amber-500 to-amber-700 rounded-full"></span>
                                <h2 class="text-xl font-bold text-white">Vision Badge</h2>
                            </div>
                            <div
                                class="bg-gray-800/40 border border-gray-700/40 rounded-2xl p-6 sm:p-8 flex items-center gap-6">
                                <div
                                    class="flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-700 text-white shadow-lg shadow-amber-900/20">
                                    <span class="text-3xl font-bold">{{ $vision->badge_value }}</span>
                                </div>
                                <div>
                                    <p class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-1">Label</p>
                                    <p class="text-white text-xl font-bold">{{ $vision->badge_label_en }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div id="content-en"
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-300 leading-relaxed text-base">{{ $vision->description_en }}</p>
                                </div>
                            </div>
                            ...
                            <div id="content-dari"
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-300 leading-relaxed text-base">{{ $vision->description_dari }}</p>
                                </div>
                            </div>
                            ...
                            <div id="content-pashto"
                                ...
                                <div>
                                    ...
                                    <p class="text-white text-2xl sm:text-3xl font-bold leading-tight">
                                        {{ $vision->title_pashto }}</p>
                                </div>
                                <div>
                                    <label
                                        class="text-gray-400 text-xs uppercase tracking-wider font-medium mb-2 block">تشریح</label>
                                    <p class="text-gray-300 leading-relaxed text-base">{{ $vision->description_pashto }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- VISION PILLARS --}}
                        @if ($vision->allPillars && $vision->allPillars->count() > 0)
                            <div class="mt-12">
                                <div class="flex items-center gap-3 mb-6">
                                    <span
                                        class="w-1.5 h-8 bg-gradient-to-b from-green-500 to-green-700 rounded-full"></span>
                                    <h2 class="text-2xl font-bold text-white">Vision Pillars</h2>
                                    <span
                                        class="ml-auto px-3 py-1 rounded-full text-xs font-medium bg-gray-800 text-gray-400 border border-gray-700">{{ $vision->allPillars->count() }}
                                        items</span>
                                </div>

                                <div class="grid md:grid-cols-2 gap-5">
                                    @foreach ($vision->allPillars as $index => $pillar)
                                        <div
                                            ...
                                            <div class="flex justify-between items-start mb-5">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <span
                                                        class="text-gray-300 text-sm font-medium">{{ $pillar->icon_name }}</span>
                                                </div>
                                                @if ($pillar->is_active)
                                                    <span
                                                        class="text-xs px-4 py-3 rounded-full bg-green-500/10 text-green-400 border border-green-500/20 font-medium">Active</span>
                                                @else
                                                    <span
                                                        class="text-xs px-4 py-3 rounded-full bg-gray-500/10 text-gray-400 border border-gray-500/20 font-medium">Inactive</span>
                                                @endif
                                            </div>
                                            <div class="space-y-4">
                                                <div>
                                                    <p
                                                        ...
                                                        English</p>
                                                    <p class="text-white text-lg font-bold mb-1">{{ $pillar->title_en }}
                                                    </p>
                                                    <p class="text-gray-200 text-sm leading-relaxed">
                                                        {{ $pillar->description_en }}</p>
                                                </div>
                                                <div class="border-t border-gray-700/50 pt-4" dir="rtl">
                                                    <p
                                                        class="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1">
                                                        دری</p>
                                                    <p class="text-white text-lg font-bold mb-1">{{ $pillar->title_dari }}
                                                    </p>
                                                    <p class="text-gray-200 text-sm leading-relaxed">
                                                        {{ $pillar->description_dari }}</p>
                                                </div>
                                                <div class="border-t border-gray-700/50 pt-4" dir="rtl">
                                                    <p
                                                        class="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1">
                                                    ...
                                                    <p class="text-white text-lg font-bold mb-1">
                                                        {{ $pillar->title_pashto }}</p>
                                                    <p class="text-gray-200 text-sm leading-relaxed">
                                                        {{ $pillar->description_pashto }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
        ...
        @else
            ...
            <div class="max-w-xl mx-auto mt-20 bg-gray-900 border border-gray-800 rounded-2xl p-10 text-center">
                ...
                <h2 class="text-xl text-white font-semibold">No Vision Found</h2>
                <p class="text-gray-400 mt-3 text-sm">Create your vision content first to see it here.</p>
                <a href="{{ route('admin.about.vision.create') }}"
