...
<div class="fixed inset-0 z-30 bg-gray-900/70 backdrop-blur-sm sm:hidden" 
     x-show="sidebarOpen" 
...
<aside class="fixed top-0 left-0 z-40 w-64 h-screen bg-gray-900 border-r border-gray-700 transition-transform duration-300 ease-in-out" 
    ...
    <ul x-show="open" x-collapse class="py-2 space-y-1">
        ...
        <li>
            ...
            <li>
                <a href="{{ route('admin.partners.index') }}" 
                   class="flex items-center p-2.5 text-white rounded-lg hover:bg-gray-700 group transition-colors {{ request()->routeIs('admin.partners.*') ? 'bg-gray-700' : '' }}">
                    <svg class="w-5 h-5 text-gray-400 transition duration-75 group-hover:text-white flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                    </svg>
                    <span class="ms-3 truncate">Partners</span>
                </a>
            </li>
            ...
            <li x-data="{ open: {{ request()->routeIs('admin.contacts.*') ? 'true' : 'false' }} }">
                <button @click="open = !open" type="button" 
                    ...
                    <span class="flex-1 ms-3 text-left whitespace-nowrap truncate">Contacts</span>
                    <span class="inline-flex items-center justify-center px-2 py-0.5 ms-2 text-xs font-medium bg-red-900 text-red-300 rounded-full flex-shrink-0">3</span>
                    <svg class="w-3 h-3 transition-transform duration-200 flex-shrink-0 ml-2" 
                         :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 10 6">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                    </svg>
        ...
        <div class="pt-4 mt-auto border-t border-gray-700">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="flex items-center w-full p-2.5 text-gray-400 rounded-lg hover:bg-gray-700 hover:text-white group transition-colors">
                    <svg class="w-5 h-5 transition duration-75 group-hover:text-white flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
